...
#else
static TIM_TypeDef* (Motor_TIM[NUM_MOTORS]) = {TIM4, TIM5, TIM9, TIM10};
static uint8_t Motor_TIM_IRQ[NUM_MOTORS] = {TIM4_IRQn, TIM5_IRQn, TIM1_BRK_TIM9_IRQn, TIM1_UP_TIM10_IRQn};
#endif
...
{
	for(int i=0; i<NUM_MOTORS; i++) {
		RCC_TIMClockCmd(Motor_TIM[i], ENABLE);
	}
	Motor_Output_Config();
